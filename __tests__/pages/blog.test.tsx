...
describe("Blog", () => {
  ...
  it("renders blog post titles in the sidebar after a successful fetch", async () => {
    ...
    render(<Blog />);
    await waitFor(() => expect(screen.getAllByText("First Post").length).toBeGreaterThanOrEqual(
      1,
    ));
  ...
  it("displays the page meta content", async () => {
    axiosMock.onGet(/\/blog/).reply(200, mockBlogPosts);
    render(<Blog />);
  ...
  it("removes the blog prefix from CMS slugs", () => {
    expect(normalizeBlogSlug("blog.touchlinehq")).toBe("touchlinehq");
    expect(normalizeBlogSlug("blog.2024.01.01.first-post")).toBe(
      "2024.01.01.first-post",
    );
    expect(normalizeBlogSlug("touchlinehq")).toBe("touchlinehq");
    expect(normalizeBlogSlug("blog")).toBe("blog");
  });
});
